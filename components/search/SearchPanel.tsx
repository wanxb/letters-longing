...
export function SearchPanel({ items }: { items: SearchItem[] }) {
  ...
  return (
    <div className="rounded-md bg-white p-4 text-ink-950 shadow-editorial ring-1 ring-ink-950/10">
      ...
      <div className="relative">
        ...
        <input
          className="focus-ring min-h-12 w-full rounded-md border border-ink-950/10 bg-paper-50 py-3 pl-11 pr-4 text-base text-ink-950 placeholder:text-ink-600"
          id="site-search"
      ...
      <div className="mt-4 divide-y divide-paper-200" role="list">
        {results.length ? (
          results.map((item) => (
            <Link className="focus-ring block rounded-md px-2 py-3 transition first:pt-2 hover:bg-moss-100" href={item.url} key={`${item.type}-${item.id}`}>
