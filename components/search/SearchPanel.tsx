...
export function SearchPanel({ items }: { items: SearchItem[] }) {
  ...
  return (
    <div className="rounded-[8px] border border-white/15 bg-white p-4 text-ink-950 shadow-[0_24px_70px_rgba(0,0,0,0.22)]">
      ...
      <div className="relative">
        ...
        <input
          className="focus-ring min-h-12 w-full rounded-full border border-paper-200 bg-paper-50 py-3 pl-11 pr-4 text-base text-ink-950 placeholder:text-ink-600"
          id="site-search"
      ...
      <div className="mt-4 divide-y divide-paper-200" role="list">
        {results.length ? (
          results.map((item) => (
            <Link className="focus-ring block rounded-md px-2 py-3 transition first:pt-2 hover:bg-paper-50" href={item.url} key={`${item.type}-${item.id}`}>
              <span className="text-xs font-semibold text-moss-700">{typeLabel[item.type]}</span>
